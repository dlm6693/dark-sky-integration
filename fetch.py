...
async def main(url_template, secret_key, loc_data):    
    ...
    async with aiohttp.ClientSession() as session:
        ...
        print(f"Fetching forecast data complete. Completion time: {str(fetch_end-start)}. Exporting to {file_name} now")
        output_json = open(file=f"{str(datetime.date.today())}_forecasts.json", mode='w+')
        json.dump(obj = htmls, fp = output_json)
        export_end = datetime.datetime.now()
        print(f"Export of forecast data complete. Completion time: {str(export_end - fetch_end)}.")
